...
class BinaryIonParser(object):
    ...
    def push(self, typeid, nextposition, nextremaining):
        self.containerstack.append(
            ContainerRec(nextpos=nextposition, tid=typeid, remaining=nextremaining)
        )
    ...
    def stepin(self):
        ...
        _assert(
            (not self.valueisnull or self.state == ParserState.AfterValue)
            and (self.valueisnull or self.state == ParserState.BeforeValue)
        )
    ...
    def parsesymboltable(self):
        ...
        while fieldtype != -1:
            if not self.valueisnull:
                _assert(
                    self.valuefieldid == SID_IMPORTS,
                    "Unsupported symbol table field id",
                )
    ...
    def readimport(self):
        ...
        if table is not None:
            ...
            if len(table.symnames) < maxid:
                self.symbols.importunknown(
                    name + "-unknown", maxid - len(table.symnames)
                )
        else:
    ...
    def lobvalue(self):
        _assert(
            ...
            "Not a LOB type: %s" % self.getfieldname(),
        )
...
SYM_NAMES = [
    ...
    + [9708, 1031, 2069, 9041, 3646, 6052, 9479, 9888, 4648, 5683]
]
...
def pkcs7unpad(msg, blocklen):
    ...
    _assert(
        msg[-paddinglen:] == bchr(paddinglen) * paddinglen,
...
OBFUSCATION_TABLE = {
    ...
    "V4": (
        0x07,
        b">\x14\x0c\x12\x10-\x13&\x18U\x1d\x05Rlt\x03!\x19\x1b\x13\x04]Y\x19,\t\x1b",
    ),
    "V5": (
        0x06,
        b'~\x18~\x16J\\\x18\x10\x05\x0b\x07\t\x0cZ\r|\x1c\x15\x1d\x11>,\x1b\x0e\x03"4\x1b\x01',
    ),
    ...
    "V18": (
        0x07,
        b"I\x0b\x0e,\x19\x1aIa\x10s\x19g\\\x1b\x11!\x18yf\x0f\t\x1d7[bSp\x03",
    ),
    "V19": (
        0x05,
    ...
    "V21": (
        0x07,
        b"e\x1d\x19|\ty\x1di|N\x13\x0e\x04\x1bj<h\x13\x15k\x12\x08=\x1f\x16~\x13l",
    ),
    ...
    "V25": (
        0x09,
        b"\x07\x14w\x1e,^y\x01:\x08\x07\x1fr\tU#j\x16\x12\x1eB\x04\x16=\x06fZ\x07\x02\x06",
    ),
...
class DrmIonVoucher(object):
            ...
            while self.drmkey.hasnext():
                ...
                if self.drmkey.getfieldname() == "algorithm":
                    ...
                    _assert(
                        ...
                        "Unknown key format: %s" % self.drmkey.stringvalue(),
                    )
    ...
    def parse(self):
        ...
        _assert(
            self.envelope.next() == TID_STRUCT
            and str.startswith(
                self.envelope.gettypename(), "com.amazon.drm.VoucherEnvelope@"
            ),
    ...
    def parsevoucher(self):
        ...
        _assert(
            self.voucher.next() == TID_STRUCT
            and self.voucher.gettypename() == "com.amazon.drm.Voucher@1.0",
...
class DrmIon(object):
    ...
    def parse(self, outpages):
        ...
        _assert(
            self.ion.next() == TID_SYMBOL and self.ion.gettypename() == "doctype",
